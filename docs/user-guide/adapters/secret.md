...
The store is also exposed directly over [REST](../api/#secrets) — `GET /api/v1/secrets` lists the caller's secret **names** (values are never returned), `PUT /api/v1/secrets/{name}` stores one — and through the SDK secret managers (`venue.secrets.set/list/delete`).
