...
- **Capability enforcement** — UCAN-based `with`/`can` policies controlling which operations an agent may invoke. The `validateWritablePath` check in the workspace operations is the natural insertion point.
- **Cross-user workspace reads** — agents reading another user's `/w/` namespace, gated by capability delegation
