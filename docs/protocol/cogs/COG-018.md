...
| `budget` | integer | Default accounting budget when the skill is loaded, in **bytes** (the same storage-size units as the agent's context budgets). A loader MAY override it. |
...
| Venue catalog | `v/skills/<name>` | Venue-installed skills, published read-only to every caller |
...
**A skill's identity is its address — the directory key (canonical path) it was loaded from.** Loading, replacement, and unloading are keyed by that path: reloading the same path **overwrites** the entry (which is how an edited tool list takes effect on reload), and a caller who wants a content-pinned skill uses an asset ref (`a/<hash>`) as the address, whose metadata value hash ([COG-5](./COG-005)) pins the body via its content binding.

The metadata value hash serves separately as a **content-equality check**, not an identity: loading a skill whose resolved metadata is identical to one already loaded under a *different* address MUST be a no-op naming the existing entry, and the `(loaded)` marker MUST match by path or by content equality, so that a skill loaded by asset hash still marks its directory line.
...
The operation is available to agents, human callers, and external clients alike, which makes a venue's skill library **publicly enumerable** in the same way its operation catalog is. It also lets an operator inject a skill index into an agent as ordinary context without activating loading — useful where an agent should know what exists but not be able to acquire it.
...
Reading a skill is a read: implementations MUST apply the ordinary read capability check for the source actually consulted — namespace read for workspace, venue, and DID paths, and asset read for content-addressed refs ([COG-13](./COG-013)).
...
- The operations a skill names are resolved **at the loading venue**. A skill is portable; the tools it names are only as portable as the catalog behind them, which is why unresolved tool refs are reported rather than fatal.
