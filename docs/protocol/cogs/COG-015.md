...
- **Authority travels only in the proof channel.** UCAN tokens are carried in the request's `ucans` array — never in operation input, which is data (persisted in job records; a credential there would leak into durable history), and never in transport headers (which do not survive hops).
