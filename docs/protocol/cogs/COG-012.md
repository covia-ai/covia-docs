...
| `foreach` | No | object | Fan the step out over a collection (see [Foreach Steps](#foreach-steps)) |
...
| `[0, "body"]` | Field `body` within step 0's output |
| `[1, "result", "name"]` | `result.name` within step 1's output |

The path is resolved **within the step's output**: `[0, "body"]` is the `body` field of whatever step 0 returned.

##### Concatenation: `["concat", spec, ...]`

Concatenates the string representation of two or more resolved input specs. Each element is resolved recursively (arrays are mapping expressions, while any non-array value (string, number, boolean) is a literal), converted to a string, and joined. `["concat", "docs/", [0, "name"]]` is therefore equivalent to the fully-wrapped `["concat", ["const", "docs/"], [0, "name"]]`:
...
##### Iteration Bindings: `["item", ...]` and `["index"]`

Valid **only inside a foreach step's `input`** (see [Foreach Steps](#foreach-steps)):

| Syntax | Resolves to |
|--------|-------------|
| `["item"]` | The current element of the foreach collection |
| `["item", "field"]` | A path within the current element |
| `["index"]` | The element's zero-based position (takes no path) |

Using either binding outside a foreach step is an error.

### Foreach Steps

A step with a `foreach` object executes its operation **once per element** of a collection, as concurrent child jobs:

```json
{
  "op": "v/ops/langchain/openai",
  "name": "Classify each record",
  "foreach": { "in": [0, "records"], "maxConcurrency": 4 },
  "input": {
    "prompt": ["concat", "Classify: ", ["item", "text"]],
    "model": ["const", "gpt-5.4-mini"]
  }
}
```
...
|-----------------|----------|-------------|
| `in` | Yes | An input-mapping expression resolving to the collection. Arrays iterate element-wise; maps and indexes iterate as `[key, value]` entry pairs. |
| `maxConcurrency` | No | Positive integer bounding concurrent iterations. MUST NOT exceed the venue's configured cap. |

Semantics:

- A single snapshot of the resolved collection fixes the iteration order and the output order.
- For each element the step's `input` is resolved with the `["item", ...]` and `["index"]` bindings available; each iteration invokes `op` as its own child job.
- The step's output is an **ordered array of the per-element outputs**, in source order; downstream steps consume it like any step output. An empty collection completes immediately with `[]`.
- On the first failing iteration the orchestrator stops admitting new iterations (in-flight ones run to completion) and the step FAILS, naming the failed element's index. Cancellation of the orchestration likewise stops admission.
- Venues MAY bound the collection size and the maximum concurrency; a `foreach` exceeding either limit is rejected before any iteration starts.
- References inside `foreach.in` and the step `input` create dependencies exactly as for ordinary steps.
...
1. **Job creation**: A new job is created for the orchestration, following the standard lifecycle ([COG-8](./COG-008)): `PENDING` at creation, advancing to `STARTED` when the orchestrator begins executing steps.
...
```json
{
  ...
  "steps": [
    { "op": "v/ops/agent/request", "status": "COMPLETE", "output": {...} },
    { "op": "v/ops/agent/request", "status": "COMPLETE", "output": {...} },
    { "op": "v/ops/agent/request", "status": "COMPLETE", "output": {...} }
  ]
}
...
```json
{
  ...
  "operation": {
    ...
    "steps": [
      {
        "op": "v/ops/agent/request",
        "input": { "agentId": ["const", "Scanner"], "input": ["input"], "timeout": ["const", 60000] }
      },
      {
        "op": "v/ops/agent/request",
        "input": { "agentId": ["const", "Validator"], "input": [0, "output"], "timeout": ["const", 60000] },
