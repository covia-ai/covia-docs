...
- **Re-delegation is audited, not prevented.** A holder of a use-ability can chain-delegate it (or simply proxy the resource); the proof chain names every delegator. Grantors who cannot accept that exposure should not grant the ability — or should grant it gated (`nb.gate`, [COG-13](./COG-013)).
