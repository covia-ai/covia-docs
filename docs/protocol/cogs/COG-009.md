...
The venue MUST support at least the following responses:

| Status | Condition | Response Body |
| ------ | --------- | ------------- |
| 202 Accepted | Message enqueued for processing | `{"id": "<job-id>", "status": "<current-status>", "queued": true}` |
| 400 Bad Request | Request body is not valid JSON | Error object |
| 401 / 403 | Caller is not authenticated / not authorised for this job | Error object |
...
| 413 Payload Too Large | Message exceeds the venue's configured size limit | Error object |
...
- Accept messages for jobs in any non-terminal state (`PENDING`, `STARTED`, `PAUSED`, `INPUT_REQUIRED`, `AUTH_REQUIRED`) — including while the job is actively processing, queuing them for later — subject to the limits a venue MAY impose below
...
This means the same job can be driven by a synchronous bridge (MCP `tools/call`) or an asynchronous client (A2A `SendMessage` + SSE) interchangeably. The job's behaviour is identical in both cases; only the client-side interaction pattern differs.
...
| `SendMessage` (no taskId) | `POST /api/v1/invoke` | Creates a new job/task |
| `SendMessage` (with taskId) | `POST /api/v1/jobs/{id}` | Sends message to existing job |
| `SendStreamingMessage` | `POST /api/v1/jobs/{id}` + `GET /api/v1/jobs/{id}/sse` | Send message, then subscribe to SSE for streaming response |
...
The A2A bridge layer on the venue translates between JSON-RPC and the REST endpoint, extracting the `taskId` and message body from `params.message`. (Per the A2A specification, `taskId` is a field of the Message object.)
...
- **[COG-11: Agent Lifecycle](./COG-011)** - Stateful agent creation, run loop, and transition functions
