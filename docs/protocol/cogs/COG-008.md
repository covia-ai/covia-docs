...
1. **Each state record is content-addressed.** The record's identifier is derived from its content (see [Decision: Asset ID Scheme](#decision-asset-id-scheme) below). Any modification to a record changes its identifier, making tampering detectable.
...
Individual state records in the chain are identified by their content-addressed hash (see [Decision: Asset ID Scheme](#decision-asset-id-scheme)).
...
The current Asset ID scheme uses SHA256 of the JSON metadata string (see [COG-5: Asset Metadata](./COG-005)). A proposal under consideration would replace this with **lattice Value IDs** — the SHA3-256 hash of the canonical binary encoding defined by the Convex lattice ([CAD003](https://docs.convex.world)).

**Implications for Jobs:**

- Job state records would be identified by their lattice Value ID rather than a SHA256 of a JSON serialisation
- The lattice automatically computes Value IDs for all stored data structures, meaning state record identification comes for free with no separate hashing step
- Structural sharing and deduplication across state records would be handled natively by the lattice

**Client verification:**

Under the current SHA256/JSON scheme, any client with a SHA256 implementation can verify an asset or state record ID. Under the proposed Value ID scheme, clients that wish to perform local verification would need a CAD003 encoding implementation. Clients that do not require local verification can rely on their home venue (or multiple venues for cross-checking) as a trusted verification endpoint.

See the corresponding note in [COG-5: Asset Metadata](./COG-005) for the full discussion of this decision and its impact across the protocol.
:::
