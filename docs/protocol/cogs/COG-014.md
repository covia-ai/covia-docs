...
- `provider` identifies the venue; the service interface advertises the agent's A2A endpoint (§1).
...
**Direct addressing.** A principal that already knows an agent's address (§1) MAY request that agent's card or interact with it directly, subject to §4.
