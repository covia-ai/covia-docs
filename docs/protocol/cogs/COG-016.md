...
- **Structured** — asks are typed (text, approval, choice, checkboxes, plus extension types such as [COG-19](./COG-019)'s `token`) so clients can render them and answers can be validated
...
| Ask | A single typed question within a request (`text`, `approval`, `choice`, `checkboxes`, or an extension type — see [COG-19](./COG-019)) |
...
Other COGs MAY extend this set with new ask types. Venues MUST reject ask types they do not implement. The current extension is the `token` ask ([COG-19](./COG-019)).
...
| `type` | string | all | One of `text`, `approval`, `choice`, `checkboxes`, or an extension type defined by another COG (currently `token`, [COG-19](./COG-019)) |
...
An extension ask type MAY declare its answer **secret**. The venue MUST replace such an answer with a non-secret placeholder in every durable carrier — the persisted record's `response.answers` and the Job `output`'s `answers` — and deliver the real value only through the extension's own output channel (see [COG-19](./COG-019)).

### Job resolution

| Event | Record `status` | Job terminal state | Job payload |
|-------|-----------------|--------------------|-------------|
| `answer` response | `answered` | `COMPLETE` | `output` = `{id, outcome, answers, comments?, comment?, grants?, token?, tokens?}` |
...
`token?` is the venue-minted grant token ([Issuance](#issuance)); `tokens?` carries transported self-sovereign tokens per [COG-19](./COG-019).
...
- A bare-path `with` in an offered grant denotes the **responder's** namespace. Venue-attested issuance MUST absolutise it to `<responderDID>/…` before signing ([COG-17](./COG-017)'s issuance rule — the venue's `iss` is not the responder, so a stored bare path would otherwise denote the venue's own namespace).
...
- The response MUST **echo** the grants being approved in its `grants` field. The venue issues exactly the intersection of (a) the echoed grants and (b) the offers actually triggered by the choices made (approved approval asks and selected options). An echoed grant that was not offered-and-triggered MUST fail the response. Echo matching is verbatim — the response repeats the offer's exact `{with, can}` (a bare-path offer is echoed bare); absolutisation happens once, at issuance.
