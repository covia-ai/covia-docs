...
This standard specifies the **capability model** for agent authorisation on the Covia Grid — how agents are granted, attenuated, disclosed, and enforced fine-grained access to operations and resources.
...
A capability `{with: "w/", can: "crud"}` can delegate `{with: "w/reports/", can: "crud/read"}` but NOT `{with: "w/", can: "crud/write"}` to a scope broader than the original.
