...
The venue signs the token with its own keypair; the issuer is the venue DID. Attenuations MUST be within the caller's own namespace.
...
6. **Root authority**: the chain root's issuer MUST be entitled to root a grant over the resource — the DID owning the resource (self-sovereign), or the venue hosting it (venue-issued / custodial). Any other root is refused
