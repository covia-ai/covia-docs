...
The `grant` family is a **meta-family**: for any ability `X`, `grant/X` is the right to *issue grants* of `X`, distinct from the right to exercise `X` (`grant/crud/read` does not cover `crud/read`, and vice versa). Granting semantics — grantable authority, issuance, and per-hop verification — are specified in [COG-17: Capability Granting](./COG-017).
...
5. If the proof chain is non-empty, each parent UCAN verified recursively; each hop MUST be issued under a granting right (`grant/<ability>`) held by its issuer, and MUST attenuate (per-hop, fail-closed at the capability — see [COG-17](./COG-017))
...
Narrowing is necessary but not sufficient: **re-delegation is opt-in**. Issuing a delegation of ability `X` requires the granting right `grant/X` — merely holding `X` does not include the right to pass it on. Resource owners hold all granting rights over their own resources implicitly. The complete granting model is specified in [COG-17: Capability Granting](./COG-017).
